...
- [ ] **Pytest and CI:** When your shell **`pytest --version`** is older than the pin in **`requirements.txt`**, ran **`make wiki-test`** or **`make wiki-all`** with **`.venv`** on **`PATH`** after **`pip install -r requirements.txt`** (**README.md** Pre-push, **`schema/wiki-quickstart.md`** **Pytest and CI**).
