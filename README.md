...
**Git remotes (two repositories).** **LLM Wiki Base Model** (this neutral template checkout): `https://github.com/tjr1974/LLM-Wiki-Base-Model.git`. **LLM Wiki Manager** (coordination and canonical toolchain home): `https://github.com/tjr1974/LLM-Wiki-Manager.git`. On this **Base Model** checkout, keep **`origin`** → `https://github.com/tjr1974/LLM-Wiki-Base-Model.git`. Add **`manager`** → `https://github.com/tjr1974/LLM-Wiki-Manager.git` when you need **`fetch`** or **cherry-picks** from **Manager**. Maintainers who work primarily in **LLM Wiki Manager** use the opposite layout (**`origin`** → Manager, **`base-model`** → this URL). See **`wiki/synthesis/llm-wiki-family-repositories.md`**. If **`origin`** still uses a legacy **`llm-wiki-base-model`** path, run **`git remote set-url origin https://github.com/tjr1974/LLM-Wiki-Base-Model.git`** so **`fetch`** and **`push`** avoid redirect noise.
